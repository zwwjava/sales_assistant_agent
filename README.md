...
本项目采用 **多智能体协同** 的设计理念，将购物助手的复杂能力拆分为多个专职子智能体，包括入口处理、聊天交互、导购路由、售后支持、人工客服、输出规范、检索服务（商品推荐）等模块。各智能体之间通过**HTTP**进行通信与协作，形成职责清晰、可扩展的系统架构。

整体进度完全看空闲时间，而且内容不会完全贴近生产情况，仅使用简单的提示词之类的。（仅模仿公司的项目）

## 进度
- [2026-04-07] 初始化购物智能体；完善意图识别跳转；
- [2026-04-02] 接postgresql，聊天记录结点.
- [2026-03-26] 初始化项目结构.
- 通用待办
  - 跨agent流式输出
  - agent调用追踪
...
负责维护常规知识库与商品推荐能力，为导购系统提供精准的检索与推荐支持。主要功能包括：

- **向量模型选型**：根据业务场景选择合适的向量化模型（如BGE、M3E、OpenAI Embedding等），平衡召回率、响应速度与资源消耗，支持文本与多模态向量的统一索引。
- **常规知识库**：维护商品百科、品牌信息、参数说明、使用教程、常见问答等结构化与半结构化知识，支持定期更新与版本管理，为RAG提供高质量的知识检索来源。
- **商品推荐**：基于支持多种推荐策略（如协同过滤、向量相似度召回、热门商品兜底），能够一次性推荐多个相关商品并附上推荐理由。
...
│       ├── input_chekc_agent   # 输出检查
│       ├── history_agents      # 历史聊天
│       └── main_cognition      # 主意图识别
│   ├── shopping_agent          # 导购智能体
│   ├── output_agent            # 输出智能体
│   └── to_human_agent          # 转人工智能体
├── config/                     # 配置
├── retriever/                  # 检索服务（API提供）
├── web/                        # web窗口
└── workflow/                   # 工作流定义
```
